...
import { categories, getCategoryBySlug, getFaqsByCategory, getArticleCount } from '../data/helpContent.js';
...
export default function HelpCategory() {
  ...
  const related = useMemo(
    () => category ? categories.filter((c) => c.id !== category.id).slice(0, 3) : [],
    [category]
  );

  if (!category) return <Navigate to={to('/help')} replace />;

  const faqs = getFaqsByCategory(category.id);
  const Icon = category.icon;
  const articleCount = faqs.length;
  ...
  return (
    <>
      <SEO
        title={`${title} — ${t('help.eyebrow')}`}
        path={`/help/${category.slug}`}
        description={`${description} ${articleCount} ${articleLabel} in the FPT Help Center.`}
      />
      <section className="relative pt-32 sm:pt-40 pb-12 overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-6 sm:px-8">
          <HelpBreadcrumbs trail={[{ label: title }]} />
          <div className="mt-6 flex items-start gap-5">
            ...
            <div className="flex-1 min-w-0">
              <div className="text-xs font-semibold uppercase tracking-[0.22em] text-[#FCA5A5]">{title}</div>
              <h1 className="mt-3 text-4xl sm:text-5xl lg:text-6xl font-semibold leading-[1.05] tracking-tight">{description}</h1>
              <p className="mt-4 text-sm font-mono-num uppercase tracking-[0.18em] text-tertiary">{articleCount} {articleLabel}</p>
            </div>
          </div>
          <div className="mt-10 max-w-2xl"><HelpSearch size="sm" /></div>
        </div>
      </section>
      <Section className="!pt-6">
        <div className="grid lg:grid-cols-[280px_1fr] gap-10 lg:gap-14">
          <CategorySidebar activeSlug={category.slug} />
          <div className="min-w-0"><FAQAccordion faqs={faqs} /></div>
        </div>
      </Section>
      <Section className="bg-[#0A0A0A]">
        <SectionHeader eyebrow={t('help.relatedTopics')} title={t('help.keepExploring')} />
